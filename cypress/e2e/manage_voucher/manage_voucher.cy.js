...
const fillVoucherForm = (data) => {
  ...
  if (data.jenis !== undefined) {
    cy.get(selectors.typeSelect).select(data.jenis);
  }
...
Then("voucher successfully saved", () => {
  ...
  cy.contains(selectors.voucherRow, voucherData.valid.kode).should("exist");
});
...
Then("system should reject voucher date", () => {
  cy.verifyNotification(
    "The tanggal mulai field must be a date after or equal to today",
  );
});
...
When("admin input negative discount value", () => {
  ...
  cy.get(selectors.valueInput).type("-");
});
...
When("admin create voucher with date {string}", (date) => {
  cy.get(selectors.addButton).click();

  fillVoucherForm({
    nama: "Voucher Test Tanggal " + date,
    kode: `DATE-${Date.now()}`,
    jenis: "Nominal (Rp)",
    nilai: "10000",
...
When("admin create voucher with discount {string}", (discount) => {
  ...
  fillVoucherForm({
    ...
    kode: `DISC-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
    jenis: "Nominal (Rp)",
